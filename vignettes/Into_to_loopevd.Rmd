...
```
The evd package provides methods for fitting different extreme value distributions (EVDs) to data. It also including non-stationary fitting and significance tests. Below we use the loopevd function raster_evd to fit Gumebel, Generlised Extreme Value (GEV) and mixed climate EVDs with the evd functions of fgumbel(), fgev and fgumbelx(). Note if parallel processes in possible with your install of terra, the raster_evd can allocate the number of cores the terra::app function uses for analysis.  

```{r, include=FALSE}

system.time(gumbel_r <- raster_evd(r,"fgumbel",silent=TRUE))

system.time(gev_r <- raster_evd(r,"fgev",silent=TRUE))

system.time(try(gumbelx_r <- raster_evd(r,"fgumbelx",silent=TRUE,ntrys = 3),silent=TRUE))
gev_r
```
the `loopevd` function `raster_evd()` function returns a spatRaster object with layers for each of the EVD paramters location(loc) scale shape and covariate matrix. 
...
```{r  out.width = '80%',fig.height=4,fig.width=6, fig.align = "center"}
gev_rl20 = terra::app(x = gev_r,fun = qevd_vector,p = 1-.05, evd_mod_str = "fgev")
...
system.time(gumbelns_r <- suppressWarnings(raster_evd(r,"fgumbel",nsloc = nsloc)))

#print("gridded non stationary gev fit:")
system.time(gevns_r <- suppressWarnings(raster_evd(r,"fgev",nsloc = nsloc)))
